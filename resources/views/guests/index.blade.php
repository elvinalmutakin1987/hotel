    ...
    <div class="content">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <a type="button" class="btn btn-outline-primary mb-3" href="{{ route('guests.create') }}"> Add New </a>

                <input type="text" id="search" name="search" class="form-control mb-3" placeholder="Search guest..."
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a class="link-primary link-offset-2 link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover me-2"
                                            href="{{ route('guests.edit', $d->id) }}">
                                            Edit
                                        </a>
                                        <form class="d-inline" action="{{ route('guests.destroy', $d->id) }}" method="POST"
                                            id="form-delete{{ $d->id }}">
                                            @csrf
                                            @method('DELETE')
                ...
                <nav>
                    {{ $guest->links('pagination::bootstrap-5') }}
                </nav>
...
@push('javascript')
    <script>
        ...
        function search() {
            var url = '{{ route('guests.index', ['search' => '_search']) }}'
