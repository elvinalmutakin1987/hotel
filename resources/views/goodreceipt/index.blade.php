    ...
    <div class="content">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <a type="button" class="btn btn-outline-primary mb-3" href="{{ route('goodreceipt.create') }}"> Add New
                </a>
                ...
                <table id="table1" class="table table-striped mt-3">
                    ...
                    <tbody class="table-group-divider">
                        @if ($goodreceipt->count() == 0)
                            <tr>
                                <td colspan="7" class="text-center">No data</td>
                            </tr>
                        @else
                            @foreach ($goodreceipt as $d)
                                <tr>
                                    <td><a class="link-primary link-offset-2 link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover me-2"
                                            href="{{ route('goodreceipt.show', $d->id) }}">{{ $d->number }}</a></td>
                                    <td>{{ $d->purchase->number }}</td>
                                    <td>{{ $d->date }}</td>
                                    ...
                                    <td class="text-center">
                                        @if ($d->status == 'Submit')
                                            <a class="link-dark link-offset-2 link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover me-2"
                                                href="{{ route('goodreceipt.print', $d->id) }}">
                                                Print
                                            </a>
                                        @else
                                            <a class="link-primary link-offset-2 link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover me-2"
                                                href="{{ route('goodreceipt.edit', $d->id) }}">
                                                Edit
                                            </a>
                                            <form class="d-inline" action="{{ route('goodreceipt.destroy', $d->id) }}"
                ...
                <nav>
                    {{ $goodreceipt->links('pagination::bootstrap-5') }}
                </nav>
