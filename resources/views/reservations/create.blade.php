            ...
            <div class="card-body">
                <form action="{{ route('reservations.store') }}" method="post">
                    ...
                    <a type="button" class="btn btn-outline-secondary"
                        href="{{ route('reservations.index', ['view' => $view]) }}">Back</a>
                </form>
            </div>
        </div>
    </div>
...
@push('javascript')
    <script>
        ...
        function get_room() {
            $.get("{{ route('reservations.get_rooms') }}", {
        ...
        $("#check_in_date").on('change', function() {
            if (checkDate(this.value)) {
                ...
                let new_date = new Date(date.getTime() + 86400000)
                const year = new_date.getFullYear()
                const month = String(new_date.getMonth() + 1).padStart(2, '0')
                const day = String(new_date.getDate()).padStart(2, '0')
                $("#check_out_date").val(`${year}-${month}-${day}`)
            } else {
                Swal.fire({
                    title: "Warning!",
                    text: "You can't pick a past date. Please select today or a future date",
                    icon: "warning"
                });
        ...
        $("#check_out_date").on('change', function() {
            if (checkDate(this.value) && checkDataInOut($("#check_in_date").val(), this.value)) {
                get_room()
            } else {
                Swal.fire({
                    title: "Warning!",
                    text: "You can't pick a past date adn less than or same time Check-in Date. Please select today or a future date",
                    icon: "warning"
                });
                let date = new Date($("#check_in_date").val())
                let new_date = new Date(date.getTime() + 86400000)
                const year = new_date.getFullYear()
                const month = String(new_date.getMonth() + 1).padStart(2, '0')
                const day = String(new_date.getDate()).padStart(2, '0')
